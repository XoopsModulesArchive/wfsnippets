<?php
/*
* $Id: snippets_new.php,v 1.1 2006/03/27 01:06:49 mikhail Exp $
* Module: Snippets
* Version: v1.03
...
function b_snippets_new_show($options)
{
    global $xoopsDB;
    ...
    $sql = 'SELECT snippetID, title, datesub, counter, groupid FROM ' . $xoopsDB->prefix('snippets') . ' ORDER BY ' . $options[0] . ' DESC';
    ...
    while ($myrow = $xoopsDB->fetchArray($result)) {
        if (checkAccess($myrow['groupid'])) {
            ...
            $wfsnipn['id'] = $myrow['snippetID'];

            if ('datesub' == $options[0]) {
                $wfsnipn['new'] = formatTimestamp($myrow['datesub'], 's');
            } elseif ('counter' == $options[0]) {
                $wfsnipn['new'] = $myrow['counter'];
            }

            $block['snipnew'][] = $wfsnipn;
        }
    }
...
function b_snippets_new_edit($options)
{
